...
def call_llm(prompt: str) -> float:
    ...
    try:
        ...
        # DEBUG: Print first 3 calls only
        if not hasattr(call_llm, 'call_count'):
            call_llm.call_count = 0
        call_llm.call_count += 1
        if call_llm.call_count <= 1:  # Sadece ilk call
            print(f"\n[LLM DEBUG {call_llm.call_count}]")
            print(f"Response: '{raw_response}'")
            print(f"Parsed score: {score}")
            print(f"\n[PROMPT DEBUG]")
            print(f"{prompt[:600]}...")  # İlk 600 char
            print(f"[PROMPT LENGTH: {len(prompt)} chars]\n")
        elif call_llm.call_count <= 3:
            print(f"\n[LLM DEBUG {call_llm.call_count}]")
            print(f"Response: '{raw_response}'")
            print(f"Parsed score: {score}")
...
def rerank(dataset_name: str, num_queries: int = 300):
    ...
    print(f"\n{'='*80}")
    print(f"Dataset: {dataset_name.upper()} | Model: Llama-3-8B | Queries: {num_queries}")
    print(f"{'='*80}")
    ...
    output_dir = f"./results/llm_reranking/llama3_8b/top20"
    
    # 1. Final rankings JSON
    with open(f"{output_dir}/{dataset_name}_llama3_8b_{timestamp}.json", "w") as f:
    ...
    with open(f"{output_dir}/{dataset_name}_llama3_8b_detailed_{timestamp}.json", "w") as f:
    ...
    with open(f"{output_dir}/{dataset_name}_llama3_8b_report_{timestamp}.txt", "w") as f:
        f.write("\n".join(report_lines))
...
if __name__ == "__main__":
    import os
    os.makedirs("./results/llm_reranking/llama3_8b/top20", exist_ok=True)
    ...
    # print(f"Check reports in: ./results/llm_reranking/llama3_8b/")
    ...
    summary_lines = [
        "="*80,
        "COMPREHENSIVE SUMMARY - LLM RE-RANKING (Llama-3-8B)",
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "="*80,
        "",
    ...
    for result in all_results:
        summary_lines.append(f"  {result['dataset']}_llama3_8b_report_{result['timestamp']}.txt")
    
    summary_lines.append("="*80)
    
    with open(f"./results/llm_reranking/llama3_8b/comprehensive_summary_{timestamp}.txt", "w") as f:
        f.write("\n".join(summary_lines))
