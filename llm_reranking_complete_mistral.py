...
def call_llm(prompt: str) -> float:
    """Call HF Mistral and parse score"""
    try:
...
def rerank(dataset_name: str, num_queries: int = 300):
    ...
    print(f"\n{'='*80}")
    print(f"Dataset: {dataset_name.upper()} | Model: Mistral-7B-Instruct-v0.3 | Queries: {num_queries}")
    print(f"{'='*80}")
    ...
    output_dir = f"./results/llm_reranking/mistral7b/top20"
    
    # 1. Final rankings JSON
    with open(f"{output_dir}/{dataset_name}_mistral7b_{timestamp}.json", "w") as f:
    ...
    with open(f"{output_dir}/{dataset_name}_mistral7b_detailed_{timestamp}.json", "w") as f:
    ...
    report_lines = [
        ...
        f"Model: Mistral-7B-Instruct-v0.3 (HuggingFace)",
    ...
    with open(f"{output_dir}/{dataset_name}_mistral7b_report_{timestamp}.txt", "w") as f:
        f.write("\n".join(report_lines))
...
if __name__ == "__main__":
    import os
    os.makedirs("./results/llm_reranking/mistral7b/top20", exist_ok=True)
    ...
    # print(f"Check reports in: ./results/llm_reranking/mistral7b/top20/")
    ...
    summary_lines = [
        "="*80,
        "COMPREHENSIVE SUMMARY - LLM RE-RANKING (Mistral-7B-Instruct-v0.3)",
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "="*80,
        "",
    ...
    for result in all_results:
        summary_lines.append(f"  {result['dataset']}_mistral7b_report_{result['timestamp']}.txt")
    
    summary_lines.append("="*80)
    
    with open(f"./results/llm_reranking/mistral7b/top20/comprehensive_summary_{timestamp}.txt", "w") as f:
        f.write("\n".join(summary_lines))
