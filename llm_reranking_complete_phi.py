...
def load_model_once():
    ...
    _TOKENIZER = AutoTokenizer.from_pretrained(HF_MODEL_ID, trust_remote_code=True)
...
def rerank(dataset_name: str, num_queries: int = 300):
    ...
    print(f"\n{'='*80}")
    print(f"Dataset: {dataset_name.upper()} | Model: Phi-3-mini-4k (3.8B) | Queries: {num_queries}")
    print(f"{'='*80}")
    ...
    output_dir = f"./results/llm_reranking/phi3/top20"
    
    # 1. Final rankings JSON
    with open(f"{output_dir}/{dataset_name}_phi3_{timestamp}.json", "w") as f:
    ...
    with open(f"{output_dir}/{dataset_name}_phi3_detailed_{timestamp}.json", "w") as f:
    ...
    report_lines = [
        ...
        f"Model: Phi-3-mini-4k-instruct (3.8B)",
    ...
    with open(f"{output_dir}/{dataset_name}_phi3_report_{timestamp}.txt", "w") as f:
        f.write("\n".join(report_lines))
...
if __name__ == "__main__":
    import os
    os.makedirs("./results/llm_reranking/phi3/top20", exist_ok=True)
    ...
    # print(f"Check reports in: ./results/llm_reranking/phi3/top20/")
    ...
    summary_lines = [
        "="*80,
        "COMPREHENSIVE SUMMARY - LLM RE-RANKING (Phi-3-mini-4k-instruct 3.8B)",
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "="*80,
        "",
    ...
    for result in all_results:
        summary_lines.append(f"  {result['dataset']}_phi3_report_{result['timestamp']}.txt")
    
    summary_lines.append("="*80)
    
    with open(f"./results/llm_reranking/phi3/top20/comprehensive_summary_{timestamp}.txt", "w") as f:
        f.write("\n".join(summary_lines))
