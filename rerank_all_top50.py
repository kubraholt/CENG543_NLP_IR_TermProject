# llm_rerank_all_models_top50.py
import json
import re
import torch
...
def rerank_single_model(model_key: str, dataset_name: str, num_queries: int = 300):
    ...
    output_dir = f"./results/llm_reranking/{model_key}/top50"
    import os
    os.makedirs(output_dir, exist_ok=True)
    
    # 1. Final rankings JSON
    with open(f"{output_dir}/{dataset_name}_{model_key}_top50_{timestamp}.json", "w") as f:
        json.dump(llm_results, f, indent=2)
    
    # 2. Generate TXT report
    report_lines = [
        "="*80,
        f"LLM RE-RANKING EVALUATION REPORT (TOP-50)",
    ...
    for metric in ["NDCG@1", "NDCG@3", "NDCG@5", "NDCG@10", "NDCG@20", "NDCG@50", "MAP@10", "MAP@20", "MAP@50", "Recall@10", "Recall@20", "Recall@50", "Recall@100", "P@3", "P@10", "MRR"]:
    ...
    with open(f"{output_dir}/{dataset_name}_{model_key}_top50_report_{timestamp}.txt", "w") as f:
        f.write("\n".join(report_lines))
...
def run_all_models():
    """Run all models on all datasets with Top-50"""
    
    print("\n" + "="*80)
    print("🚀 LLM RE-RANKING - ALL MODELS - TOP-50 COMPARISON")
    print("="*80)
    ...
    summary = {
        "experiment": "LLM Re-ranking - All Models - Top-50",
        "date": datetime.now().isoformat(),
        "top_k": TOP_K,
        "results": all_results
    }
    
    with open(f"./results/llm_reranking/all_models_top50_summary_{timestamp}.json", "w") as f:
        json.dump(summary, f, indent=2)
    
    # Create comprehensive TXT summary
    summary_lines = [
        "="*80,
        "COMPREHENSIVE SUMMARY - LLM RE-RANKING (ALL MODELS - TOP-50)",
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "="*80,
        "",
    ...
    for result in all_results:
        summary_lines.append(f"  {result['dataset']}_{result['model']}_top50_report_{result['timestamp']}.txt")
    
    summary_lines.append("="*80)
    
    with open(f"./results/llm_reranking/all_models_top50_summary_{timestamp}.txt", "w") as f:
        f.write("\n".join(summary_lines))
    
    print("\n" + "="*80)
    print("📊 FINAL RESULTS - TOP-50 RE-RANKING")
    print("="*80)
    ...
    print("="*80)
    print(f"✅ JSON summary: all_models_top50_summary_{timestamp}.json")
    print(f"✅ TXT summary: all_models_top50_summary_{timestamp}.txt")
    print("="*80)
